// Indexing + query logic for the PalCodex MCP.
...
const DEFAULT_SITE_URL = 'https://doodesch-mods.github.io/PalCodex';

// Human-friendly section labels for the top-level folders.
const SECTION_LABELS = {
  '': 'Introduction',
  'getting-started': 'Getting Started',
  frameworks: 'API & Framework Mods',
  'tools-and-setup': 'Tools & Setup',
  'asset-modding': 'Asset, Model & Texture Modding',
  'data-modding': 'Data & DataTable Modding',
  'lua-scripting': 'Lua Scripting',
  animations: 'Animations',
  audio: 'Audio Modding',
  packaging: 'Packaging & Pak Pipeline',
  translations: 'Translations',
  'dedicated-server': 'Dedicated Server Modding',
  troubleshooting: 'Troubleshooting & FAQ',
  reference: 'Reference & Links',
  contributors: 'Contributors',
};

// Valid curation states (see the Astro content.config.ts schema).
export const VERSION_STATES = ['verified-1.0', 'needs-review', 'legacy-ea'];
...
export function loadCorpus(options = {}) {
  const docsDir = options.docsDir || process.env.PALCODEX_DOCS_DIR || DEFAULT_DOCS_DIR;
  const siteUrl = (
    options.siteUrl ||
    process.env.PALCODEX_SITE_URL ||
    DEFAULT_SITE_URL
  ).replace(/\/$/, '');
  ...
  for (const file of files) {
    ...
    pages.push({
      ...
      versionStatus: data.versionStatus || 'verified-1.0',
...
export function search(corpus, query, { limit = 8, section, versionStatus } = {}) {
  let results = corpus.mini.search(query);
  if (section) results = results.filter((r) => r.section === section);
  if (versionStatus) results = results.filter((r) => r.versionStatus === versionStatus);
...
export function getPage(corpus, input) {
  ...
  return {
    ...
    versionStatus: page.versionStatus,
    sourceDate: page.sourceDate || undefined,
    lastReviewed: page.lastReviewed || undefined,
    sources: page.sources,
    markdown: page.markdown,
  };
}

export function listPages(corpus, { section, versionStatus } = {}) {
  return corpus.pages
    .filter((p) => (section ? p.section === section : true))
    .filter((p) => (versionStatus ? p.versionStatus === versionStatus : true))
    .sort(
      (a, b) =>
    ...
    .map((p) => ({
      ...
      url: p.url,
      versionStatus: p.versionStatus,
      sourceDate: p.sourceDate || undefined,
